...
---
- Python + FastAPI + Redis + Celery 기반의 STT 및 감정 분석 파이프라인을 직접 설계하고, 혼자 개발부터 배포까지 전 과정을 경험
...
- AudioWorklet 내에 VAD(Vocal Activity Detection) 필터를 적용하여 무음 구간을 선제적으로 제거함으로써, 네트워크 부하와 STT 비용을 효율적으로 절감
---
## 📌 프로젝트 개요
...
FastAPI 서버
        ↓
Celery Task (transcribe_audio) 호출
        ↓
stt_worker (음성 인식 처리 - OpenAI Whisper)
        ↓
Celery Task (analyze_text) 호출
        ↓
analyzer_worker (감정 분석 처리 - Huggingface Transformers)
        ↓
Redis Pub/Sub (result_channel로 발행)
        ↓
listener_service (result_channel 구독,
                  통계 처리 후 메시지를 result_messages, final_stats에 각각 발행)
        ↓
FastAPI (result_messages, final_stats 각각 구독)
        ↓
    WebSocket 전송
        ↓
Browser 실시간 결과 업데이트
...
### 1️⃣0️⃣ FastAPI on_event() 개선 → redis_subscriber task 안정화
